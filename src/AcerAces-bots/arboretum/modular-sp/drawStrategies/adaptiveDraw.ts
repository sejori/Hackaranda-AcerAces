...
export function adaptiveDraw(state: playerState<move>): move {
	...
	const filteredOptions = options.filter(option => {
		if (currentDrawSource === 'deck' && option === drawingMove.Deck) return false;
		if (currentDrawSource === 'own' && option === drawingMove.OwnDiscard) return false;
		if (currentDrawSource === 'opponent' && option === drawingMove.OpponentDiscard) return false;
		return true;
	});
