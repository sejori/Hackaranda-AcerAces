...
export function strategicPlay(state: playerState<move>): move {
	...
	for (const card of availableCards) {
		for (const coord of emptySpaces) {
			...
			const rightToScoreAdjustment = calculateRightToScoreAdjustment(
				card, 
				rightToScoreAnalysis, 
				saveSpecies, 
				state
			);
...
function analyzeRightToScoreCompetition(state: playerState<move>): Record<species, {
	ourScore: number;
	...
	const analysis: Record<species, any> = {} as any;
	...
	for (const species of SPECIES) {
		...
		for (const card of state.opponentHand) {
			if (card && card[0] === species) {
				...
				if (card[1] === 1) opponentScore += 1;
				if (card[1] === 8) opponentScore += 2;
			}
		}
...
function getAvailableCardsForPlay(
	playCards: any[], 
	saveCards: any[], 
	saveSpecies: species[], 
	rightToScoreAnalysis: any, 
...
function calculateRightToScoreAdjustment(
	card: any, 
	rightToScoreAnalysis: any, 
	saveSpecies: species[], 
	state: playerState<move>
...
function tryRainbowStaircase(state: playerState<move>): move | null {
...
function calculateBlockingPenalty(playArea: any, placedCard: any, coord: any): number {
	let penalty = 0;
	const [x, y] = coord;
