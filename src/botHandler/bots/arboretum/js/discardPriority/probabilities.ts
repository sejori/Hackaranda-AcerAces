...
export function getCardsByPriority(cards: Hand) {
  const priority = (process.env.priority ?? "12345678").split(
    "",
  ) as unknown as rank[];
  // console.error({ priority });
  const rankCards: Record<rank, Hand> = {
    2: [],
    1: [],
    3: [],
    4: [],
  ...
  for (let card of cards) {
    rankCards[card[1]].push(card);
  }
  // console.error({ rankCards });
  for (let rank of priority) {
    // console.error({ rank, cards: rankCards[rank] });
    // console.error(rankCards[rank]);
    if (rankCards[rank].length > 0) {
      return rankCards[rank];
    }
  }
  return cards;
}
